...
static void EINK_Lut(uint8_t *lut)
{
...
//lut
static void EINK_SetLut(uint8_t *lut)
{
	EINK_Lut(lut);
    ...
    EINK_SendCommand(0x03);
    EINK_SendData(lut[154]);
	
    EINK_SendCommand(0x04);
    EINK_SendData(lut[155]);
	EINK_SendData(lut[156]);
	EINK_SendData(lut[157]);
	
	EINK_SendCommand(0x2c);
